...
class UserController extends Controller
{
    ...
    {
    $user = User::findOrFail($id);
    //$status = Role::all()->pluck('name');
    $status = User::where("Status", "NOT LIKE", "%Suspend%")->get();
    return view('users.status', compact('user', 'status'));
    }
    ...
    public function setStatus(Request $request, $id)
    {
        ...
        $user->save();
        return redirect()->back()->with(['success' => 'Status Sudah Di Set']);
    }
